<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Jadoo - Travel Agency</title>
    <link rel="stylesheet" href="styles.css">
    <link href="https://fonts.googleapis.com/css2?family=Nunito:wght@400;600;700&display=swap" rel="stylesheet">
</head>
<body>
    <header>
        <nav>
            <div class="logo">Jadoo</div>
            <ul>
                <li><a href="#">Destinations</a></li>
                <li><a href="#">Hotels</a></li>
                <li><a href="#">Flights</a></li>
                <li><a href="#">Bookings</a></li>
            </ul>
            <div class="auth-buttons">
                <a href="#" class="login">Login</a>
                <a href="#" class="signup">Sign Up</a>
            </div>
        </nav>
        <div class="hero">
            <h1>Travel, enjoy and live a new and full life</h1>
            <p>Built Wicket longer admire do barton vanity itself do in it. Preferred to sportsmen it engrossed listening. Park gate sell they west hard for the.</p>
            <a href="#" class="btn-primary">Find out more</a>
            <a href="#" class="btn-secondary">Play Demo</a>
        </div>
    </header>

    <section class="services">
        <h2>We Offer Best Services</h2>
        <div class="service-items">
            <div class="service-item">
                <img src="images/weather-icon.png" alt="Calculated Weather">
                <h3>Calculated Weather</h3>
                <p>Barton vanity itself do in it. Preferred to men it engrossed listening.</p>
            </div>
            <div class="service-item">
                <img src="images/flight-icon.png" alt="Best Flights">
                <h3>Best Flights</h3>
                <p>Engrossed listening. Park gate sell they west hard for the.</p>
            </div>
            <div class="service-item">
                <img src="images/event-icon.png" alt="Local Events">
                <h3>Local Events</h3>
                <p>Barton vanity itself do in it. Preferred to men it engrossed listening.</p>
            </div>
            <div class="service-item">
                <img src="images/customization-icon.png" alt="Customization">
                <h3>Customization</h3>
                <p>We deliver outsourced aviation services for military customers.</p>
            </div>
        </div>
    </section>

    <section class="destinations">
        <h2>Top Destinations</h2>
        <div class="destination-cards">
            <div class="destination-card">
                <img src="images/rome.jpg" alt="Rome, Italy">
                <div class="destination-info">
                    <h3>Rome, Italy</h3>
                    <p>$5.42k</p>
                    <p>10 Days Trip</p>
                </div>
            </div>
            <div class="destination-card">
                <img src="images/london.jpg" alt="London, UK">
                <div class="destination-info">
                    <h3>London, UK</h3>
                    <p>$4.2k</p>
                    <p>12 Days Trip</p>
                </div>
            </div>
            <div class="destination-card">
                <img src="images/europe.jpg" alt="Full Europe">
                <div class="destination-info">
                    <h3>Full Europe</h3>
                    <p>$15k</p>
                    <p>28 Days Trip</p>
                </div>
            </div>
        </div>
    </section>

    <section class="booking">
        <h2>Book Your Next Trip In 3 Easy Steps</h2>
        <ul>
            <li><span>1.</span> Choose Destination</li>
            <li><span>2.</span> Make Payment</li>
            <li><span>3.</span> Reach Airport on Selected Date</li>
        </ul>
        <div class="ongoing-trip">
            <h3>Trip To Greece</h3>
            <p>14-29 June | by Robin K</p>
            <div class="progress-bar">
                <div class="progress" style="width: 40%;"></div>
            </div>
            <p class="people-going">24 people going</p>
            <a href="{{ route ('about') }}"class="text=1xl font-bold mb-4">Selengkapnya...</a>
        </div>
    </section>

    <section class="testimonials">
        <h2>What People Say About Us</h2>
        <div class="testimonial-cards">
            <div class="testimonial-card">
                <p>"On the Windows talking painted pasture yet its express parties use. Sure last upon he same as knew next. Of believed or diverted no."</p>
                <h4>Mike Taylor</h4>
                <p>Lahore, Pakistan</p>
            </div>
            <div class="testimonial-card">
                <p>"On the Windows talking painted pasture yet its express parties use. Sure last upon he same as knew next. Of believed or diverted no."</p>
                <h4>Chris Thomas</h4>
                <p>CEO of Red Button</p>
            </div>
        </div>
    </section>

    <footer>
        <div class="partners">
            <img src="images/axon.png" alt="Axon">
            <img src="images/jetstar.png" alt="Jetstar">
            <img src="images/expedia.png" alt="Expedia">
            <img src="images/qantas.png" alt="Qantas">
            <img src="images/alitalia.png" alt="Alitalia">
        </div>
        <div class="subscribe">
            <h3>Subscribe to get information, latest news and other interesting offers about Jadoo</h3>
            <form>
                <input type="email" placeholder="Your email">
                <button type="submit">Subscribe</button>
            </form>
        </div>
        <div class="footer-info">
            <p>&copy; All rights reserved Jadoo.co</p>
            <ul class="footer-links">
                <li><a href="#">About</a></li>
                <li><a href="#">Careers</a></li>
                <li><a href="#">Mobile</a></li>
                <li><a href="#">Help/FAQ</a></li>
                <li><a href="#">Press</a></li>
                <li><a href="#">Affiliates</a></li>
            </ul>
            <div class="social-media">
                <a href="#"><img src="images/facebook-icon.png" alt="Facebook"></a>
                <a href="#"><img src="images/twitter-icon.png" alt="Twitter"></a>
                <a href="#"><img src="images/instagram-icon.png" alt="Instagram"></a>
            </div>
        </div>
    </footer>
</body>
</html>